...
export default function AnalyticsScreen() {
  ...
  const categoryMinutes = useMemo(() => {
    ...
    focusSessions.forEach((s) => {
      if (s.category) {
        map[s.category] = (map[s.category] ?? 0) + s.completedMinutes;
      }
    });
    return map;
  }, [focusSessions]);
  ...
  const last7Days = useMemo(() => {
    return Array.from({ length: 7 }, (_, i) => {
      ...
      return { date: d.toLocaleDateString("en", { weekday: "short" }), total: dayTasks.length, done };
    });
  }, [tasks]);
  ...
  const renderScoreTab = () => (
    <View style={styles.tabContent}>
        ...
        {Object.entries(scoreBreakdown).map(([key, val]) => (
          <View key={key} style={styles.breakdownRow}>
            ...
            <View style={styles.breakdownBar}>
              <MiniBar value={val} max={100} color={colors.primary} />
            </View>
      ...
      <View style={[styles.statCards, { gap: 10 }]}>
  ...
  const renderScreenTimeTab = () => (
    <View style={styles.tabContent}>
      <View style={[styles.card, { backgroundColor: colors.card, borderColor: colors.border }]}>
        <Text style={[styles.cardTitle, { color: colors.foreground }]}>Today's Focus</Text>
        <View style={styles.screenTimeRow}>
          <View style={styles.screenTimeStat}>
            <Text style={[styles.bigNumber, { color: colors.primary }]}>{Math.floor(todayFocusMinutes / 60)}h {todayFocusMinutes % 60}m</Text>
            <Text style={[styles.screenTimeLabel, { color: colors.mutedForeground }]}>productive today</Text>
          </View>
          <View style={[styles.riskBadge, { backgroundColor: "#10B98122", borderColor: "#10B98144" }]}>
            <Text style={[styles.riskValue, { color: colors.success }]}>34</Text>
            <Text style={[styles.riskLabel, { color: colors.success }]}>Risk Score</Text>
          </View>
        </View>
      </View>
      ...
      <View style={[styles.card, { backgroundColor: colors.card, borderColor: colors.border }]}>
        <View style={styles.dangerZoneHeader}>
          ...
          <Text style={[styles.cardTitle, { color: colors.foreground }]}>
            Your Danger Zone: {dangerZone.dangerHours.map((h) => `${h}:00`).join("–")}
          </Text>
        </View>
        <Text style={[styles.dangerNote, { color: colors.mutedForeground }]}>
          {dangerZone.isBootstrapEstimate ? "Based on typical patterns (7 days needed for personal data)" : `Based on ${dangerZone.dataPointsCount} days of your data`}
        </Text>
      ...
      <View style={[styles.card, { backgroundColor: colors.card, borderColor: colors.border }]}>
        <Text style={[styles.cardTitle, { color: colors.foreground }]}>Top Distraction Apps</Text>
...
const styles = StyleSheet.create({
  ...
  statCards: { flexDirection: "row" },
  miniStatCard: { flex: 1, borderRadius: 14, borderWidth: 1, padding: 16, alignItems: "center", gap: 4 },
  miniStatValue: { fontSize: 22, fontFamily: "Inter_700Bold" },
  miniStatLabel: { fontSize: 11, fontFamily: "Inter_400Regular", textAlign: "center" },
  ...
  riskBadge: { borderRadius: 12, borderWidth: 1, padding: 16, alignItems: "center" },
  ...
  dangerNote: { fontSize: 12, fontFamily: "Inter_400Regular", marginTop: -6 },
